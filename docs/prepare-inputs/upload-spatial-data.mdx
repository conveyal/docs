...
Enter a name for the spatial dataset source, then select the appropriate file(s). Click the upload button to start the upload. Datasets will be saved with the source name you entered and the attribute/column names of the uploaded files.

Uploaded files are converted to Conveyal's custom grid format. The default [spatial resolution](/analysis/methodology#spatial-resolution) of the grid cells is set to zoom level z9. Higher grid cell resolutions are available, but they should be used [with caution](/analysis/methodology#zoom-levels). Selecting zoom levels that yield an average of fewer than one opportunity per grid cell may lead to unexpected results. After processing is complete you can select a dataset from the dropdown menu to show the dot-density map derived from it at the selected resolution.
...
For projects in the United States, employment data from from the _Longitudinal Employer-Household Dynamics_ ([LEHD](https://lehd.ces.census.gov/)) Program can be imported with a one-click import function: <Button variantColor='cyan'><FaUsers /> Fetch LODES (2018)</Button>
