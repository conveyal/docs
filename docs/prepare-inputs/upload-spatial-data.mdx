...
Uploaded files are converted to Conveyal's custom grid format. The default [spatial resolution](/analysis/methodology#spatial-resolution) of the grid cells is set to zoom level z9. Higher grid cell resolutions are available, but they should be used [with caution](/analysis/methodology#zoom-levels). After processing is complete you can select a dataset from the dropdown menu to show the dot-density map derived from it at the selected resolution.
