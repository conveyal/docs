...
The zoom level sets the [spatial resolution](/analysis/methodology#spatial-resolution) for converting the dataset source to Conveyal's custom grid format. The default is zoom level z9. Higher grid cell resolutions are available, but they should be used [with caution](/analysis/methodology#zoom-levels), especially for large or low density data sources. The number of resulting grid cells is limited to 5 million; for given geographic extents, each increment in zoom level increases the number of resulting grid cells by a factor of 4. And selecting zoom levels that yield an average of fewer than one opportunity per grid cell may lead to unexpected results. 

Click the upload button to start the upload. Datasets will be saved with the source name you entered and the attribute/column names of the uploaded files. After processing is complete you can select a dataset from the dropdown menu to show the dot-density map derived from it at the selected resolution.
...
For projects in the United States, employment data from from the _Longitudinal Employer-Household Dynamics_ ([LEHD](https://lehd.ces.census.gov/)) Program can be imported with a one-click import function: <Button variantColor='cyan'><FaUsers /> Fetch LODES (2019)</Button>
