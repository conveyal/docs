...
- `"buildGridsForModes":` Additional on-street modes (other than walk) for which to pre-build large data tables (defaults to `["WALK"]`). Setting this to `["BICYCLE", "CAR"]` will require extra time to calculate and save bike and car-related data tables when the network is first built but eliminate repeated re-building of these tables when subsequent routing servers start up.
...
Any custom options set in the network can be viewed in the read-only "Advanced bundle configuration" section in the left sidebar. If you need to change any of these options, you can create a new network bundle that re-uses the same OSM and GTFS, pasting the modified options into the configuration section for the new network.