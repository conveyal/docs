...
* Polygon Shapefile → Aggregation Area 
...
Then, provide a name and select all components: .shp, .dbf, .shx, and .prj files. To derive an aggregation area from a shapefile, it must fit within a bounding box smaller than 250,000 sq. km, must not have any feature with area exceeding 2 square degrees, and should preferably use unprojected WGS84 coordinates. 
