...
### Requesting path details

To view the details of transit paths in your browser console, pin the blue circular destination marker on the map and add `recordPaths: true` to the custom request JSON. Running regional analyses with `recordPaths` or `recordTimes` set to `true` and custom origins selected will trigger the experimental features described [here](/guides/compute-freeform).
