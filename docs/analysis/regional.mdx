...
If you want to show regional accessibility results with custom color bins or on your own basemap, you can download the results. The download button allows you to save displayed regional analysis results in a [GeoTIFF](https://en.wikipedia.org/wiki/GeoTIFF) raster format. These files can then be opened in GIS software to create custom maps or [compute custom indicators](/guides/process-raster-results). Downloading results also allows you to see the raw [grid cells](/analysis/methodology#spatial-resolution) used for analysis, rather than the smoother interpolated results shown in your browser.
