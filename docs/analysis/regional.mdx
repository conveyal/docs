...
After clicking the <Button variantColor='green'><FaPlus /> Regional analysis</Button>, you will see options to configure your regional analysis. Give the analysis a name, select at least one Destination opportunity layer, and ensure the list of travel time cutoffs and percentiles will cover the results you want. By default, each point from a rectangular grid will be used as an origin in this analysis. If you have uploaded [freeform (non-grid) spatial datasets](/prepare-inputs/upload-spatial-data#freeform-non-grid-points), you can instead use the exact coordinates of these points as origins. Whether origins are derived from a default rectangular grid or a freeform spatial dataset, the total number of origins for the regional analysis will be displayed. Finally, click <Button variantColor='green'><FaPlus /> Create</Button> to start the analysis.
...
For regional analyses run with freeform origins, travel time, accessibility, or path results may also be available for download; these results are not shown on the map.
