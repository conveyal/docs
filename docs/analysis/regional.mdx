...
Once results for a single origin are displayed, the <Button variantColor='green'>Regional analysis <FaPlus /></Button> will be enabled. This button is disabled unless isochrones are displayed; checking single-origin isochrone results is a verification step that helps avoid heavy computation for analyses with invalid settings.

### Result types

Different result types are available in regional analysis, including cumulative-opportunity access to opportunities, origin-destination travel times, origin-destination path details, and dual access. Available result types depend on the selected origin type (freeform vs. grid), and they can be activated below the origin selector or with specific fields in the custom request settings JSON in the analysis panel. Accessibility output is enabled by default. Options for other types, such as travel time matrices, are described [here](/guides/compute-freeform).
...
If you select a rectangular grid as the origin pointset, results will be raster maps showing the number of destinations reachable from each point in that grid.
...
If you select a freeform (non-grid) origin pointset, results can be downloadable CSV files with [access, travel time, and path results](/guides/compute-freeform). These options require a previously created [freeform (non-grid) spatial dataset](/prepare-inputs/upload-spatial-data#freeform-non-grid-points).
