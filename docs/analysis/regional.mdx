...
* Dual access: time needed to reach a threshold number of destinations. For more details, see [this paper](https://doi.org/10.1111/gean.12220) or the [TransitCenter Equity Dashboard](https://dashboard.transitcenter.org), which reports the time needed to reach the nearest hospital and the time needed to reach the third-nearest grocery store from various origins. If this option is selected, you will also see a field to enter the desired thresholds. For example, 3 would yield time needed to reach third-nearest grocery store with grocery stores selected as the destination layer, and 10,000 would yield the time needed to reach 10,000 jobs with jobs selected as the destination layer.
