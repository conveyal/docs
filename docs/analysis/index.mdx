...
If this is your first time performing an analysis with a given [network bundle](/glossary#network-bundle) it may take some time to build the network, however this only needs to be done once for each bundle. For more information, see the [FAQ page](/learn-more/faq#how-long-should-it-take-to-start-a-single-origin-analysis).

Once the compute cluster is initialized you should see an isochrone displayed in blue around your point on the map. If you have selected an destination opportunity layer, you will also see a [chart of accessibility](#chart-of-accessibility-results) at selected time and percentile thresholds. You may also select a comparison project and scenario, which will be shown in red.
...
When an opportunity layer is selected, the histogram above the map shows how many destinations are reachable at the current travel time cutoff and percentile. Adjust the sliders to explore different thresholds and watch the chart update instantly. Hovering over the chart highlights the percentile and displays the exact count of reachable opportunities at that level.

The display of modifications on the map is controlled in editing mode (See: [Toggling display of modifications](/edit-scenario/usage#toggling-display-of-modifications)). To change what modifications are shown, you must set the visibility there and then navigate back to the analysis page. 
