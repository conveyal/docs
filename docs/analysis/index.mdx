...
Pinning the destination marker also enables the transit itinerary viewer. To view itinerary details, pin a destination on the map, click "Fetch Results," then click "View routes to destination" in the origin legend (upper left) after results are fetched. 
