...
#### Raster cost
Conveyal provides certain cost functions that translate exposure to a raster surface along a network edge to modified traversal time for that edge. For example, loading a geotiff elevation raster [data source](/prepare-data) and setting `costFunction: "TOBLER"` in the custom JSON will apply Tobler's hiking function to walking travel times.
