...
## Travel time matrix
Setting `recordTimes:true` in the custom analysis request JSON will result in a CSV file with a row for each origin point, destination point, and travel time percentile. This setting requires freeform pointsets to be selected for both the Origin points and the Destination opportunity layer. 
