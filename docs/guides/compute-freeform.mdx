...
1. Upload a CSV file and [process it as a freeform (non-grid) spatial dataset](/prepare-inputs/upload-spatial-data#freeform-non-grid-points). Each analysis is currently limited to 4 million destinations for any given origin, and 16 million total origin-destination pairs.
