...
## Accessibility
Setting `recordAccessibility:true` will result in a CSV file with a row for each origin point, destination opportunity layer, travel time percentile, and cutoff. For example:
...
## Travel time matrix
Setting `recordTimes:true` will result in a CSV file with a row for each origin point, destination point, and travel time percentile. This setting requires freeform pointsets to be selected for both the Origin points and the Destination opportunity layer. For example:
