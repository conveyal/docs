...
You may also want compute a single composite index, or a complex indicator combining different logical conditions or spatial layers. The Conveyal team is considering ways to support additional holistic indicators directly in the interface; contact yoru support team if you have specific requests or ideas. In the meantime, you can compute a wider range of indicators by downloading results and running custom calculations in offline GIS software (we recommend QGIS). By default, travel time results, regional access results, and uploaded spatial datasets are all available for download as grid (raster) files with consistent resolutions. This guide provides a few examples of ways to use these raster files.

## Percent difference between two scenarios

The Conveyal interface shows the absolute difference between two scenarios:
<Image
  src='/img/custom-indicators/regional-comparison.png'
  alt='Map showing increase in access from proposed infrastructure in Los Angeles'
/>

If you instead want a map of the percent difference, to highlight places where the change in access is large relative to the baseline, follow these steps:

1. Download GeoTIFF results for each completed regional analysis from the respective results pages.
1. Open both downloaded GeoTIFF files as raster layers in GIS software, and rename them for convenience (e.g. `scenarioId` and `baselineId`).
1. Use the GIS raster calculator, with an expression to calculate the percent difference: `("scenarioId@1" - "baselineId@1") / "baselineId@1"`. The `@1` indicates the first band of each raster.
<Image
  src='/img/custom-indicators/raster-calculator.png'
  alt='Using the QGIS Raster Calculator with an expression for the percent difference'
/>
1. Style the result layer accordingly. You may need to set reasonable min/max values and bins.
<Image
  src='/img/custom-indicators/percent-difference-result.png'
  alt='Styling the result layer.'
/>
...
If you want to calculate the total number of households (or residents etc.) who benefit from a given scenario, we recommend running analyses with a [logistic decay function](/learn-more/decay-functions#logistic-cdf), then following the steps below.

For background, the default [decay function](/learn-more/decay-functions) in Conveyal is a step function, representing a hard limit at the selected travel time cutoff. Consider a new link that reduces travel time from a given origin to many destinations by 5 minutes. From this origin, destinations that were previously 25 minutes away (and are now 20 minutes away), or 45 minutes away (and now 40) would not be included when calculating the impact of the new link. The only benefits with a step function and a 30 minute cutoff would arise from opportunities that were previously reachable in over 30 minutes, and are now reachable in under 30 minutes. To avoid limiting results in this way, Conveyal supports gravity-style access indicators with different decay functions. With these functions, closer destinations are considered more valuable, so reducing travel time to any destination yields a benefit.

1. Repeat the first two steps above.
1. Download the household layer as a GeoTIFF (.tif) file from the spatial datasets page.
<Image
  src='/img/custom-indicators/households.png'
  alt='Spatial dataset download'
/>
...
<Image
  src='/img/custom-indicators/configure-regional.png'
  alt='Configuring a regional analysis with multiple destination types and cutoffs'
/>
...
<Image
  src='/img/custom-indicators/composite-raster-load.png'
  alt='Loading raster layers into QGIS'
/>
...
<Image
  src='/img/custom-indicators/healthcare-threshold.png'
  alt='Using the QGIS Raster Calculator to calculate which areas meet a certain threshold'
/>
...
<Image
  src='/img/custom-indicators/clip-raster.png'
  alt='Clipping a raster layer to a vector boundary'
/>
...
<Image
  src='/img/custom-indicators/job-access.png'
  alt='Regional analysis summary showing that approximately 52% of city residents can access the target number of jobs within 45 minutes'
/>
...
<Image
  src='/img/custom-indicators/combined-population.png'
  alt='Multiplying a combined logical condition by the clipped population layer'
/>
