...
1. Pre-build tables for biking and driving. This approach generally works besst when using separate network bundles for transit and non-transit analysis; include only a small placeholder GTFS feed (with a minimal number of stops) in the non-transit analysis. Then add this to the Advanced bundle config when creating a bundle (more [here](/prepare-inputs#advanced-bundle-configuration)):
